...
// ─── สร้างข้อความสำหรับออเดอร์ใหม่ (ส่ง Group — มีลิงก์รายคน) ───────────────
export function buildNewOrderMessage({ order, memberSummaries, appUrl }) {
  ...
  // แสดงยอด + ลิงก์เฉพาะคนในข้อความ Group
  const lines = memberSummaries.map(
    (s) => `👤 ${s.member_name}: ฿${s.total_due}\n🔗 ${appUrl}/pay/${order.id}?token=${s.access_token}`
  ).join('\n\n')
  ...
  return {
    ...
    text: [
      ...
      lines,
      ``,
      `📊 ดูยอดค้างทั้งหมดของตัวเอง กดที่ลิงก์ชื่อตัวเองด้านบน`,
    ].join('\n'),
  }
}

// ─── ข้อความทวงรวม ส่ง Group (รายชื่อทุกคนที่ยังไม่จ่าย) ──────────────────────
export function buildGroupReminderMessage({ order, unpaidMembers, appUrl }) {
  const deadline  = new Date(order.deadline).toLocaleDateString('th-TH', { year: 'numeric', month: 'long', day: 'numeric' })
  const isOverdue = order.status === 'overdue'

  const lines = unpaidMembers.map(
    m => `👤 ${m.member_name}: ฿${m.total_due}\n🔗 ${appUrl}/pay/${order.id}?token=${m.access_token}`
  ).join('\n\n')

  return {
    type: 'text',
    text: [
      isOverdue ? `🔴 เกินกำหนดแล้ว! ออเดอร์ ${order.order_number}` : `⚠️ ยังไม่จ่ายค่าน้ำ ออเดอร์ ${order.order_number}`,
      `⏰ กำหนด: ${deadline}`,
      ``,
      `คนที่ยังไม่จ่าย (${unpaidMembers.length} คน):`,
      lines,
    ].join('\n'),
  }
}

// ─── ข้อความเตือนรายคน (push 1:1) — มี token ในลิงก์ ─────────────────────────
export function buildReminderMessage({ order, memberSummary, appUrl }) {
  const deadline = new Date(order.deadline).toLocaleDateString('th-TH', {
    year: 'numeric', month: 'long', day: 'numeric',
  })
  const isOverdue = order.status === 'overdue'
  const payLink = `${appUrl}/pay/${order.id}?token=${memberSummary.access_token}`
  ...
  return {
    ...
    text: [
      ...
      `กำหนด: ${deadline}`,
      isOverdue ? `❗ กรุณาจ่ายโดยด่วน` : ``,
      ``,
      `📎 กดลิงก์นี้เพื่ออัปสลิป:`,
      payLink,
